...
from subtask_A.data_set import label_to_class
...
with open('RelQuestion_to_RelComment_stats.csv') as csvfile:
    # load data
    Y = []
    X = []
    reader = csv.DictReader(csvfile)
    for row in reader:
        similarity = (float(row["w2v_cosine_similarity"]))
        relevance = row["relevance"]
        X.append(similarity)
        Y.append(label_to_class(relevance))
    X = np.array(X)
    Y = np.array(Y)
    ...
    # plots
    plt.ylabel('subtaskA cosine similarity')
